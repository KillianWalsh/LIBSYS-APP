...
async function renderData() {
    ...
    books.forEach(book => {
        ...
        detailButton.innerHTML = "Detailjer";
...
async function populateData(title) {
    ...
    for (const book of bookData) {
        ...
        await getUserEmail()
            .then(function (data) {
                borrowerId = data[Object.keys(data)[0]]
        ...
        if (borrowerId !== undefined) {
            ...
            if (book.status !== "IN_STOCK") {
                ...
                var tdButton2 = createNode("td");
                tdButton2.appendChild(loanButton);
                loanButton.innerHTML = "Loan";
                loanButton.value = book.id;
                loanButton.onclick = function () {
                    console.log("Låna");
                    saveLoan(book.id, book.title);
...
async function saveLoan(bookId, bookTitle) {
    ...
    if (response.ok) {
        alert("You successfully loaned this book!");
        clearTable(document.getElementById('LoanTableBody'))
        await populateData(bookTitle)
    } else {
